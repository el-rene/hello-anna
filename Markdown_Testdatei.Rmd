...
Insgemsamt wurden im Zeitverlauf **`r anzahl_verschiedene_produkte`** verschiedene Produkte bestellt. Das umsatzstärkste Produkt ist **`r product_summary %>% head(1) %>% .[2]`** mit **`r product_summary %>%  head(1) %>% .[5]` Umsatz**, darauf folgen **`r product_summary %>% .[2] %>% slice(2)`** mit **`r product_summary %>% .[5] %>% slice(2)` Umsatz** und **`r product_summary %>% .[2] %>% slice(3)`** mit **`r product_summary %>% .[5] %>% slice(3)` Umsatz**. Das Produkt `r product_summary %>% .[2] %>% slice(3)` ist zudem das teuerste Produkt des Unternehmens mit einem Kaufpreis von `r product_summary %>% .[4] %>% slice(3)` Pfund.
...
ggplot(product_summary, aes(unit_price, revenue)) +
  geom_point() +
  labs(title = "Umsatz eines Produktes im Verhältnis zu dessen Produktpreis") # ?????? Hier nur für die Ausreißer Legende hinzufügen
```
