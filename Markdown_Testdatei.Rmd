...
```{r Manipulation der Rohdaten, include=FALSE}
...
Schauen wir uns die Bestellzeit für jeden Tag einzeln an, dann sehen wir, dass an allen Tagen die meisten Bestellungen in Stunde 12 eingehen. Außerdem sehen wir, dass donnerstags vergleichsweise länger bestellt wird.4
...
ggplot(product_summary, aes(unit_price, anzahl)) +
  geom_point() +
  labs(title = "Verkaufte Menge eines Produktes im Verhältnis zu dessen Produktpreis") # log dazu
...
data(pareto_analyse_produkt)
ggplot(pareto_analyse_produkt, aes(CustomerID, cum.revenue)) + geom_col(aes(fill=Kategorie)) 
...
hist(size(transaktionsdaten), col = "lightblue")

```

Aus dem Histogram geht hervor, dass der Großteil der Bestellungen eine geringe Anzahl an Artikeln enthält. 
Betracht man den folgenden Boxplot, so wird deutlich, dass es viele Ausreißer nach oben gibt. 
...
boxplot(size(transaktionsdaten), col = "lightblue", main="Boxplot of Transaction data")
...
head(regeln, n = 10, by = "confidence")
