...
```{r Libraries laden, include=FALSE, message=FALSE}
...
Die nachfolgende Analyse beruht auf einem E-Commerce Datensatz, bereitgestellt durch das Center of Machine Learning and Intelligent Systems, abrufbar unter folgendem Link:
<https://archive.ics.uci.edu/ml/datasets/online+retail#>

Die Daten beziehen sich auf einen Online Händler, der in UK ansässig ist und eine breite Palette verschiedenster Geschenk- und Dekoartikel vertreibt. 
Der Datensatz beinhaltet dabei Bestellungen aus einer Vielzahl an Ländern. Die vorliegende Analyse wurde auf Bestellungen aus UK beschränkt. 

Der Zeitraum der Analyse bezieht sich auf den `r min(bereinigte_daten$date)` bis `r max(bereinigte_daten$date)`. Im Datensatz sind **`r anzahl_kunden` Kunden** enhalten, die gesamt **`r gesamtbestellungen_kunden` Bestellungen** getätigt haben und dabei einen **Gesamtumsatz von `r gesamtumsatz_kunden`** erzeugt haben.
...
weekday_summary <- bereinigte_daten %>%
  ...
  summarise(anzahl_bestellungen = n(),
            revenue = sum(revenue), 
            bestellwert = revenue/anzahl_bestellungen)%>% 
...
```{r,product_summary, echo=FALSE}
# Produktpreise

produkt_preise_summary <- bereinigte_daten %>%
  group_by(Country) %>%
  summarise(mittelwert_produktpreis = mean(UnitPrice),
            median_produktpreis = median(UnitPrice),
            min_produktpreis = min(UnitPrice),
            max_produktpreis = max(UnitPrice))%>%
  group_by(Country)

kable(produkt_preise_summary) %>% 
  kable_styling()
